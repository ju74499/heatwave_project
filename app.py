...
DB_PATH = "final.db"
DISTRICT_GEOJSON_PATH = "data/seoul_district_boundary.geojson"
DONG_GEOJSON_PATH = "data/seoul_adm_dong_boundary.geojson"
...
if district_geojson is None:
    st.warning("자치구 경계 GeoJSON 파일이 없어 지도 시각화를 건너뜁니다. data/seoul_district_boundary.geojson 파일을 추가하면 지도가 표시됩니다.")
else:
    map_df_sql = """
    SELECT
        district,
...
if dong_geojson is None:
    st.warning("행정동 경계 GeoJSON 파일이 없어 행정동 지도 시각화를 건너뜁니다. data/seoul_adm_dong_boundary.geojson 파일을 추가하면 지도가 표시됩니다.")
else:
